...
def train_model():
    ...
    clf = RandomForestClassifier(
        n_estimators=1000, n_jobs=-1, min_samples_leaf=9, max_features=40, oob_score=True, min_impurity_decrease=0.000015)
    clf.fit(X, y)
    ...
    with open("Reports/pssm_forest_scoredump.report", "a+") as dh:
        dh.write(str(window) + " PSSM RandomForest 320 trees + min_impurity_decrease=0.000015, max feat 40, min leaf 15  " + str(now.strftime("%Y-%m-%d %H:%M:%S")) +
